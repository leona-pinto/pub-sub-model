...
app.get('/', (req, res) => {
  res.sendFile(path.join(__dirname, 'public', 'smartHome-modular.html'));
});
...
async function startTvSubscriber() {
  ...
  // await lightsConsumer.subscribe({ topic: KAFKA_TOPIC2, fromBeginning: false });
    ...
    eachMessage: async ({ topic, partition, message }) => {
      try {
        ...
        if (distance !== null && temperature !== null) {
          ...
          graphQueue.push({
            deviceName: "lights",
            state: deviceState.devices.lights,
            temperature,
            distance,
            graphIRI: GRAPH_LIGHTS,
          });
          processGraphQueue();
